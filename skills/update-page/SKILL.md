...
- **Modern Layout**: Use Tailwind utility classes for modern aesthetics (e.g., responsive grids, flexbox, glassmorphism `bg-white/60 backdrop-blur-md`, `rounded-xl`, `shadow-lg`). Do not output bare HTML tags without styling. **UI Styling Convention**: Dark background colors (e.g., `bg-gray-800`, `bg-slate-900`, `bg-black`) are strictly forbidden in generated pages as they clash with the default light-blue template themes.
