...
- **Modern Layout**: Use Tailwind utility classes for modern aesthetics (e.g., responsive grids, flexbox, glassmorphism `bg-white/60 backdrop-blur-md`, `rounded-xl`, `shadow-lg`). Do not output bare HTML tags without styling. **CRITICAL: NEVER use dark background colors (e.g., `bg-gray-800`, `bg-slate-900`, `bg-black`) for any elements, panels, or summary modules. They will clash with the default light-blue template themes. Always use light, transparent, or white/blue-tinted backgrounds (e.g., `bg-white`, `bg-blue-50`, `bg-slate-50`).**
