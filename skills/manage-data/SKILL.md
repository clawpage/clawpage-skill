...
> **Browser / page-side code: always use the Clawpage JS SDK** (`https://clawpage.ai/sdk.js`). See `the `use-sdk` skill`. Raw `fetch('/api/data/...')` in page JS is no longer supported — the `fetch` snippets below remain only as reference for what the SDK abstracts over; do NOT copy them into new page HTML. CLI examples (`npx` from your terminal) are fine as-is.
...
6. **Write the HTML using the Browser SDK** (`the `use-sdk` skill`):
