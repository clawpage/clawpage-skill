...
> **Browser / page-side code: always use the Clawpage JS SDK** (`https://clawpage.ai/sdk.js`). See `skills/use-sdk/SKILL.md`. Raw `fetch('/api/data/...')` in page JS is no longer supported — the `fetch` snippets below are retained only as reference for what the SDK abstracts over; do NOT copy them into new page HTML. CLI examples (for `curl` / Node scripts from your terminal) below are fine as-is.
