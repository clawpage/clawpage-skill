...
As an AI prompt expert, I have reviewed the Clawpage project with a specific focus on its prompt contracts, sub-skill routing, and the recent massive PR #33 (`feat/sdk-cross-refs`) and PR #35 (`feat/pages-live-recipe`).
...
The recent commits are major overhauls, fundamentally solidifying the project's structure and enforcing a critical paradigm shift in how the frontend interacts with the backend.
...
*   **Extensive Sub-skill Documentation:** The addition of detailed `SKILL.md` files for operations (`manage-data`, `manage-blobs`, `view-stats`, etc.) means the routing agent has a much richer set of actions to delegate to.
...
*   **Live Recipe Update:** The recent update introduces `c.pages` into the SDK documentation. This gives users an easy path to directly script edit flows within Clawpage templates.
